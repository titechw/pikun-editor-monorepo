# @pikun/extension-details

## 3.10.1

### Patch Changes

- Updated dependencies [3564e7c]
  - @pikun/core@3.10.1
  - @pikun/extension-text-style@3.10.1
  - @pikun/pm@3.10.1

## 3.10.0

### Patch Changes

- Updated dependencies [4aa9f57]
- Updated dependencies [4aa9f57]
  - @pikun/core@3.10.0
  - @pikun/extension-text-style@3.10.0
  - @pikun/pm@3.10.0

## 3.9.1

### Patch Changes

- @pikun/core@3.9.1
- @pikun/extension-text-style@3.9.1
- @pikun/pm@3.9.1

## 3.9.0

### Patch Changes

- Updated dependencies [bbb8e16]
  - @pikun/core@3.9.0
  - @pikun/extension-text-style@3.9.0
  - @pikun/pm@3.9.0

## 3.8.0

### Patch Changes

- @pikun/core@3.8.0
- @pikun/extension-text-style@3.8.0
- @pikun/pm@3.8.0

## 3.7.2

### Patch Changes

- @pikun/core@3.7.2
- @pikun/extension-text-style@3.7.2
- @pikun/pm@3.7.2

## 3.7.1

### Patch Changes

- @pikun/core@3.7.1
- @pikun/extension-text-style@3.7.1
- @pikun/pm@3.7.1

## 3.7.0

### Patch Changes

- Updated dependencies [35645d9]
- Updated dependencies [35645d9]
- Updated dependencies [35645d9]
  - @pikun/core@3.7.0
  - @pikun/extension-text-style@3.7.0
  - @pikun/pm@3.7.0

## 3.6.7

### Patch Changes

- @pikun/core@3.6.7
- @pikun/extension-text-style@3.6.7
- @pikun/pm@3.6.7

## 3.6.6

### Patch Changes

- @pikun/core@3.6.6
- @pikun/extension-text-style@3.6.6
- @pikun/pm@3.6.6

## 3.6.5

### Patch Changes

- Updated dependencies [1e4caea]
  - @pikun/core@3.6.5
  - @pikun/extension-text-style@3.6.5
  - @pikun/pm@3.6.5

## 3.6.4

### Patch Changes

- @pikun/core@3.6.4
- @pikun/extension-text-style@3.6.4
- @pikun/pm@3.6.4

## 3.6.3

### Patch Changes

- Updated dependencies [67f7b4a]
  - @pikun/core@3.6.3
  - @pikun/extension-text-style@3.6.3
  - @pikun/pm@3.6.3

## 3.6.2

### Patch Changes

- @pikun/core@3.6.2
- @pikun/extension-text-style@3.6.2
- @pikun/pm@3.6.2

## 3.6.1

### Patch Changes

- @pikun/core@3.6.1
- @pikun/extension-text-style@3.6.1
- @pikun/pm@3.6.1

## 3.6.0

### Patch Changes

- Updated dependencies [c0190bd]
  - @pikun/core@3.6.0
  - @pikun/extension-text-style@3.6.0
  - @pikun/pm@3.6.0

## 3.5.3

### Patch Changes

- Updated dependencies [04a0f34]
  - @pikun/extension-text-style@3.5.3
  - @pikun/core@3.5.3
  - @pikun/pm@3.5.3

## 3.5.2

### Patch Changes

- @pikun/core@3.5.2
- @pikun/extension-text-style@3.5.2
- @pikun/pm@3.5.2

## 3.5.1

### Patch Changes

- @pikun/core@3.5.1
- @pikun/extension-text-style@3.5.1
- @pikun/pm@3.5.1

## 3.5.0

### Patch Changes

- @pikun/core@3.5.0
- @pikun/extension-text-style@3.5.0
- @pikun/pm@3.5.0

## 3.4.6

### Patch Changes

- Updated dependencies [968016f]
  - @pikun/core@3.4.6
  - @pikun/extension-text-style@3.4.6
  - @pikun/pm@3.4.6

## 3.4.5

### Patch Changes

- Updated dependencies [0226d42]
- Updated dependencies [37af83b]
- Updated dependencies [f598ac7]
  - @pikun/core@3.4.5
  - @pikun/extension-text-style@3.4.5
  - @pikun/pm@3.4.5

## 3.4.4

### Patch Changes

- Updated dependencies [00cf1d7]
  - @pikun/core@3.4.4
  - @pikun/extension-text-style@3.4.4
  - @pikun/pm@3.4.4

## 3.4.3

### Patch Changes

- Updated dependencies [1ea8906]
  - @pikun/core@3.4.3
  - @pikun/extension-text-style@3.4.3
  - @pikun/pm@3.4.3

## 3.4.2

### Patch Changes

- @pikun/core@3.4.2
- @pikun/extension-text-style@3.4.2
- @pikun/pm@3.4.2

## 3.4.1

### Patch Changes

- Updated dependencies [46fa8b8]
  - @pikun/extension-text-style@3.4.1
  - @pikun/core@3.4.1
  - @pikun/pm@3.4.1

## 3.4.0

### Patch Changes

- d3773c7: Fixed an issue where the input method editor (IME) failed to handle Chinese text correctly when entering the first word. Users can now input multiple words as expected.
- Updated dependencies [895c73f]
- Updated dependencies [ad51daa]
  - @pikun/core@3.4.0
  - @pikun/extension-text-style@3.4.0
  - @pikun/pm@3.4.0

## 3.3.1

### Patch Changes

- @pikun/core@3.3.1
- @pikun/extension-text-style@3.3.1
- @pikun/pm@3.3.1

## 3.3.0

### Patch Changes

- Updated dependencies [5423726]
- Updated dependencies [5423726]
  - @pikun/core@3.3.0
  - @pikun/extension-text-style@3.3.0
  - @pikun/pm@3.3.0

## 3.2.2

### Patch Changes

- @pikun/core@3.2.2
- @pikun/extension-text-style@3.2.2
- @pikun/pm@3.2.2

## 3.2.1

### Patch Changes

- Updated dependencies [6a2873f]
  - @pikun/core@3.2.1
  - @pikun/extension-text-style@3.2.1
  - @pikun/pm@3.2.1

## 3.2.0

### Patch Changes

- Updated dependencies [5056e3e]
  - @pikun/core@3.2.0
  - @pikun/extension-text-style@3.2.0
  - @pikun/pm@3.2.0

## 3.1.0

### Patch Changes

- @pikun/core@3.1.0
- @pikun/extension-text-style@3.1.0
- @pikun/pm@3.1.0

## 3.0.9

### Patch Changes

- @pikun/core@3.0.9
- @pikun/extension-text-style@3.0.9
- @pikun/pm@3.0.9

## 3.0.8

### Patch Changes

- @pikun/core@3.0.8
- @pikun/extension-text-style@3.0.8
- @pikun/pm@3.0.8

## 3.0.7

### Patch Changes

- @pikun/core@3.0.7
- @pikun/extension-text-style@3.0.7
- @pikun/pm@3.0.7

## 3.0.6

### Patch Changes

- Updated dependencies [2e71d05]
  - @pikun/core@3.0.6
  - @pikun/extension-text-style@3.0.6
  - @pikun/pm@3.0.6

## 3.0.5

### Patch Changes

- @pikun/core@3.0.5
- @pikun/extension-text-style@3.0.5
- @pikun/pm@3.0.5

## 3.0.4

### Patch Changes

- Updated dependencies [7ed03fa]
  - @pikun/core@3.0.4
  - @pikun/extension-text-style@3.0.4
  - @pikun/pm@3.0.4

## 3.0.3

### Patch Changes

- Updated dependencies [75cabde]
  - @pikun/core@3.0.3
  - @pikun/extension-text-style@3.0.3
  - @pikun/pm@3.0.3

## 3.0.2

### Patch Changes

- @pikun/core@3.0.2
- @pikun/extension-text-style@3.0.2
- @pikun/pm@3.0.2

## 3.0.1

### Minor Changes

- 7ac01ef: We open sourced our basic pro extensions

  This release includes the following extensions that were previously only available in our Pro version:

  - `@pikun/extension-drag-handle`
  - `@pikun/extension-drag-handle-react`
  - `@pikun/extension-drag-handle-vue-2`
  - `@pikun/extension-drag-handle-vue-3`
  - `@pikun/extension-emoji`
  - `@pikun/extension-details`
  - `@pikun/extension-file-handler`
  - `@pikun/extension-invisible-characters`
  - `@pikun/extension-mathematics`
  - `@pikun/extension-node-range`
  - `@pikun/extension-table-of-contents`
  - `@pikun/extension-unique-id`

### Patch Changes

- da7ccd9: Removed details summary and details content and include it in details
- Updated dependencies [1b4c82b]
- Updated dependencies [1e91f9b]
- Updated dependencies [a92f4a6]
- Updated dependencies [8de8e13]
- Updated dependencies [20f68f6]
- Updated dependencies [5e957e5]
- Updated dependencies [89bd9c7]
- Updated dependencies [f77cbac]
- Updated dependencies [0bad53e]
- Updated dependencies [d0fda30]
- Updated dependencies [0e3207f]
- Updated dependencies [37913d5]
- Updated dependencies [28c5418]
- Updated dependencies [32958d6]
- Updated dependencies [12bb31a]
- Updated dependencies [9f207a6]
- Updated dependencies [412e1bd]
- Updated dependencies [062afaf]
- Updated dependencies [ff8eed6]
- Updated dependencies [0b4981c]
- Updated dependencies [704f462]
- Updated dependencies [3b4e06c]
- Updated dependencies [95b8c71]
- Updated dependencies [8c69002]
- Updated dependencies [664834f]
- Updated dependencies [ac897e7]
- Updated dependencies [087d114]
- Updated dependencies [32958d6]
- Updated dependencies [fc17b21]
- Updated dependencies [62b0877]
- Updated dependencies [e20006b]
- Updated dependencies [5ba480b]
- Updated dependencies [d6c7558]
- Updated dependencies [062afaf]
- Updated dependencies [9ceeab4]
- Updated dependencies [32958d6]
- Updated dependencies [bf835b0]
- Updated dependencies [4e2f6d8]
- Updated dependencies [32958d6]
  - @pikun/extension-text-style@3.0.1
  - @pikun/core@3.0.1
  - @pikun/pm@3.0.1

## 3.0.0-beta.30

### Patch Changes

- @pikun/core@3.0.0-beta.30
- @pikun/extension-text-style@3.0.0-beta.30
- @pikun/pm@3.0.0-beta.30

## 3.0.0-beta.29

### Patch Changes

- @pikun/core@3.0.0-beta.29
- @pikun/extension-text-style@3.0.0-beta.29
- @pikun/pm@3.0.0-beta.29

## 3.0.0-beta.28

### Patch Changes

- @pikun/core@3.0.0-beta.28
- @pikun/extension-text-style@3.0.0-beta.28
- @pikun/pm@3.0.0-beta.28

## 3.0.0-beta.27

### Patch Changes

- Updated dependencies [412e1bd]
  - @pikun/core@3.0.0-beta.27
  - @pikun/extension-text-style@3.0.0-beta.27
  - @pikun/pm@3.0.0-beta.27

## 3.0.0-beta.26

### Patch Changes

- Updated dependencies [5ba480b]
  - @pikun/core@3.0.0-beta.26
  - @pikun/extension-text-style@3.0.0-beta.26
  - @pikun/pm@3.0.0-beta.26

## 3.0.0-beta.25

### Patch Changes

- Updated dependencies [4e2f6d8]
  - @pikun/core@3.0.0-beta.25
  - @pikun/extension-text-style@3.0.0-beta.25
  - @pikun/pm@3.0.0-beta.25

## 3.0.0-beta.24

### Patch Changes

- @pikun/core@3.0.0-beta.24
- @pikun/extension-text-style@3.0.0-beta.24
- @pikun/pm@3.0.0-beta.24

## 3.0.0-beta.23

### Patch Changes

- @pikun/core@3.0.0-beta.23
- @pikun/extension-text-style@3.0.0-beta.23
- @pikun/pm@3.0.0-beta.23

## 3.0.0-beta.22

### Patch Changes

- @pikun/core@3.0.0-beta.22
- @pikun/extension-text-style@3.0.0-beta.22
- @pikun/pm@3.0.0-beta.22

## 3.0.0-beta.21

### Patch Changes

- Updated dependencies [813674c]
- Updated dependencies [fc17b21]
  - @pikun/core@3.0.0-beta.21
  - @pikun/extension-text-style@3.0.0-beta.21
  - @pikun/pm@3.0.0-beta.21

## 3.0.0-beta.20

### Patch Changes

- @pikun/core@3.0.0-beta.20
- @pikun/extension-text-style@3.0.0-beta.20
- @pikun/pm@3.0.0-beta.20

## 3.0.0-beta.19

### Patch Changes

- Updated dependencies [9ceeab4]
  - @pikun/core@3.0.0-beta.19
  - @pikun/extension-text-style@3.0.0-beta.19
  - @pikun/pm@3.0.0-beta.19

## 3.0.0-beta.18

### Patch Changes

- @pikun/core@3.0.0-beta.18
- @pikun/extension-text-style@3.0.0-beta.18
- @pikun/pm@3.0.0-beta.18

## 3.0.0-beta.17

### Patch Changes

- Updated dependencies [e20006b]
  - @pikun/core@3.0.0-beta.17
  - @pikun/extension-text-style@3.0.0-beta.17
  - @pikun/pm@3.0.0-beta.17

## 3.0.0-beta.16

### Patch Changes

- Updated dependencies [ac897e7]
- Updated dependencies [bf835b0]
  - @pikun/core@3.0.0-beta.16
  - @pikun/extension-text-style@3.0.0-beta.16
  - @pikun/pm@3.0.0-beta.16

## 3.0.0-beta.15

### Patch Changes

- Updated dependencies [087d114]
  - @pikun/core@3.0.0-beta.15
  - @pikun/extension-text-style@3.0.0-beta.15
  - @pikun/pm@3.0.0-beta.15

## 3.0.0-beta.14

### Patch Changes

- Updated dependencies [95b8c71]
  - @pikun/core@3.0.0-beta.14
  - @pikun/extension-text-style@3.0.0-beta.14
  - @pikun/pm@3.0.0-beta.14

## 3.0.0-beta.13

### Patch Changes

- @pikun/core@3.0.0-beta.13
- @pikun/extension-text-style@3.0.0-beta.13
- @pikun/pm@3.0.0-beta.13

## 3.0.0-beta.12

### Patch Changes

- da7ccd9: Removed details summary and details content and include it in details
  - @pikun/core@3.0.0-beta.12
  - @pikun/extension-text-style@3.0.0-beta.12
  - @pikun/pm@3.0.0-beta.12

## 3.0.0-beta.11

### Patch Changes

- @pikun/core@3.0.0-beta.11
- @pikun/extension-text-style@3.0.0-beta.11
- @pikun/pm@3.0.0-beta.11

## 3.0.0-beta.10

### Minor Changes

- 7ac01ef: We open sourced our basic pro extensions

  This release includes the following extensions that were previously only available in our Pro version:

  - `@pikun/extension-drag-handle`
  - `@pikun/extension-drag-handle-react`
  - `@pikun/extension-drag-handle-vue-2`
  - `@pikun/extension-drag-handle-vue-3`
  - `@pikun/extension-emoji`
  - `@pikun/extension-details`
  - `@pikun/extension-file-handler`
  - `@pikun/extension-invisible-characters`
  - `@pikun/extension-mathematics`
  - `@pikun/extension-node-range`
  - `@pikun/extension-table-of-contents`
  - `@pikun/extension-unique-id`

### Patch Changes

- @pikun/core@3.0.0-beta.10
- @pikun/extension-text-style@3.0.0-beta.10
- @pikun/pm@3.0.0-beta.10
