import { type Extensions, type JSONContent, getSchema } from '@pikun/core';
import { Node } from '@pikun/pm/model';
...
export function generateHTML(doc: JSONContent, extensions: Extensions): string {
  if (typeof window !== 'undefined') {
    throw new Error(
      'generateHTML can only be used in a Node environment\nIf you want to use this in a browser environment, use the `@pikun/html` import instead.'
    );
  }
