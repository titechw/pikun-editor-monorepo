...
import type { DOMOutputSpecArray, Extensions, JSONContent } from '@pikun/core';
import type { DOMOutputSpec, Mark, Node } from '@pikun/pm/model';
...
export function domOutputSpecToHTMLString(
  ...
  if (typeof content === 'object' && 'length' in content) {
    ...
    if (typeof attrs === 'object') {
      ...
      if (children === 0) {
        return (child) =>
          `<${tag}${serializeAttrsToHTMLString(attrs)}>${serializeChildrenToHTMLString(
            child
          )}</${tag}>`;
      }
